<?php
/**
 * Brooklyn Tri Customizer functionality
 *
 * @package WordPress
 ...
 */
function brooklyntri_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'brooklyntri_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'brooklyntri' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => brooklyntri_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'brooklyntri' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'brooklyntri' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'brooklyntri' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'brooklyntri' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'brooklyntri' );
}
add_action( 'customize_register', 'brooklyntri_customize_register', 11 );

/**
 * Register color schemes for Brooklyn Tri.
 *
 * Can be filtered with {@see 'brooklyntri_color_schemes'}.
 *
 ...
 */
function brooklyntri_get_color_schemes() {
	return apply_filters( 'brooklyntri_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'brooklyntri' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'brooklyntri_get_color_scheme' ) ) :
/**
 * Get the current Brooklyn Tri color scheme.
 *
 * @since Brooklyn Tri 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function brooklyntri_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = brooklyntri_get_color_schemes();
...
endif; // brooklyntri_get_color_scheme

if ( ! function_exists( 'brooklyntri_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for Brooklyn Tri.
 *
 * @since Brooklyn Tri 1.0
 *
 * @return array Array of color schemes.
 */
function brooklyntri_get_color_scheme_choices() {
	$color_schemes                = brooklyntri_get_color_schemes();
...
endif; // brooklyntri_get_color_scheme_choices

if ( ! function_exists( 'brooklyntri_sanitize_color_scheme' ) ) :
/**
 ...
 */
function brooklyntri_sanitize_color_scheme( $value ) {
	$color_schemes = brooklyntri_get_color_scheme_choices();
...
endif; // brooklyntri_sanitize_color_scheme
...
add_action( 'wp_enqueue_scripts', 'brooklyntri_color_scheme_css' );
 ...
 * @since Brooklyn Tri 1.0
 */
function brooklyntri_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', brooklyntri_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'brooklyntri_customize_control_js' );
 ...
 * @since Brooklyn Tri 1.0
 */
function brooklyntri_customize_preview_js() {
	wp_enqueue_script( 'brooklyntri-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'brooklyntri_customize_preview_js' );
 ...
 */
function brooklyntri_get_color_scheme_css( $colors ) {
 ...
 * @since Brooklyn Tri 1.0
 */
function brooklyntri_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-brooklyntri-color-scheme">
		<?php echo brooklyntri_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'brooklyntri_color_scheme_css_template' );
