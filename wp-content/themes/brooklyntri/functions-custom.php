...
function get_btc_registrants( $form_id, $event_id ) {
	...
	$sql = 'select group_concat(value, ",") as racers from ' . $wpdb->prefix . 'rg_lead_detail where form_id = ' . $form_id . ' and field_number in (1,2) and lead_id in (select lead_id from ' . $wpdb->prefix . 'rg_lead_detail where field_number=' . EVENT_FIELD_ID . ' and value = \'' . $event_id . '\' )';
